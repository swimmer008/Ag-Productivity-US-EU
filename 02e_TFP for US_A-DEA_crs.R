...
# -------------------------------------------- #
  ...
  ADEA.level_crs <- rbind(ADEA_pacific, ADEA_mountain, ADEA_nplains, 
                          ADEA_splains, ADEA_cornbelt, ADEA_southeast, 
                          ADEA_northeast, ADEA_lake, ADEA_appalacian, 
                          ADEA_delta)
  
  #replace TT
  ADEA.level_crs$TT <- ADEA.level_crs$PROF / ADEA.level_crs$TFP
  
  # Order results by state
  ADEA.level_crs <- ADEA.level_crs[order(ADEA.level_crs$state),]
  
  # Save results for comparison in "03h_TFP for US_comparison"
  save(ADEA.level_crs, file="R_output/ADEA.level_crs.Rda")
...
# PROFITABILITY 
  ...
  # PROF1960
  data_tables$PROF1960 <- ADEA.level_crs$PROF[ADEA.level_crs$year==1960]
  # PROF04
  data_tables$PROF2004 <- ADEA.level_crs$PROF[ADEA.level_crs$year==2004]
  # DPROF
...
# TERMS OF TRADE 
  ...
  # TT1960
  data_tables$TT1960 <- ADEA.level_crs$TT[ADEA.level_crs$year==1960]
  # TT04
  data_tables$TT2004 <- ADEA.level_crs$TT[ADEA.level_crs$year==2004]
  # DTT
...
# TOTAL FACTOR PRODUCTIVITY 
  ...
  # TFP1960
  data_tables$TFP1960 <- ADEA.level_crs$TFP[ADEA.level_crs$year==1960]
  # TFP04
  data_tables$TFP2004 <- ADEA.level_crs$TFP[ADEA.level_crs$year==2004]
  # DTFP
...
# MAXIMM TOTAL FACTOR PRODUCTIVITY 
  ...
  # MP1960
  data_tables$MP1960 <- ADEA.level_crs$MP[ADEA.level_crs$year==1960]
  # MP04
  data_tables$MP2004 <- ADEA.level_crs$MP[ADEA.level_crs$year==2004]
  # DMP
...
# TFP EFFICIENCY 
  ...
  # TFPE1960
  data_tables$TFPE1960 <- ADEA.level_crs$TFPE[ADEA.level_crs$year==1960]
  # TFPE04
  data_tables$TFPE2004 <- ADEA.level_crs$TFPE[ADEA.level_crs$year==2004]
  # DTFPE
...
# OUTPUT TECHNICAL EFFICIENCY 
  ...
  # OTE1960
  data_tables$OTE1960 <- ADEA.level_crs$OTE[ADEA.level_crs$year==1960]
  # OTE04
  data_tables$OTE2004 <- ADEA.level_crs$OTE[ADEA.level_crs$year==2004]
  # DOTE
...
# OUTPUT SCALE EFFICIENCY 
  ...
  # OSE1960
  data_tables$OSE1960 <- ADEA.level_crs$OSE[ADEA.level_crs$year==1960]
  # OSE04
  data_tables$OSE2004 <- ADEA.level_crs$OSE[ADEA.level_crs$year==2004]
  # DOSE
...
# OUTPUT MIX EFFICIENCY 
  ...
  # OME1960
  data_tables$OME1960 <- ADEA.level_crs$OME[ADEA.level_crs$year==1960]
  # OME04
  data_tables$OME2004 <- ADEA.level_crs$OME[ADEA.level_crs$year==2004]
  # DOME
...
# OUTPUT SCALE MIX EFFICIENCY 
  ...
  # OSME1960
  data_tables$OSME1960 <- ADEA.level_crs$OSME[ADEA.level_crs$year==1960]
  # OSME04
  data_tables$OSME2004 <- ADEA.level_crs$OSME[ADEA.level_crs$year==2004]
  # DOSME
...
# Save US-average for comparison in 03h_TFP for US_comparison.R
  ...
  Summary_adea_US_crs <- list(data_tables$DTFP[data_tables$state=="US48"],
                              data_tables$DMP[data_tables$state=="US48"],
                              data_tables$DTFPE[data_tables$state=="US48"])
  names(Summary_adea_US_crs) <- c("TFP", "TC", "TFPE") 
  save(Summary_adea_US_crs, file = "R_output/Summary_adea_US_crs.Rda")
...
# Write Table: "Profitability, TFP, and efficiency change in US agriculture (1960--2004) using the A-DEA index)
  ...
  # Write table  
  Tab_ADEA_ProfDecomp_US_crs <- kable(data_ADEA_ProfDecomp, booktabs = T, 
                                      digits = 2,
                                      row.names = FALSE,
                                      escape = FALSE,
                                      linesep = "",
                                      caption = "Profitability, TFP, and efficiency change in US agriculture (1960--2004) using the A-DEA index under CRS.",
                                      label = "Tab_ADEA_ProfDecomp_US_crs",
                                      col.names = c('State', 
                                                    '1960', '2004', "$\\Delta$",
                                                    '1960', '2004', "$\\Delta$",
                                                    '1960', '2004', "$\\Delta$",
  ...
  # Print Latex file
  writeLines(Tab_ADEA_ProfDecomp_US_crs, "Tables/Tab_ADEA_ProfDecomp_US_crs.tex")
  
  
# Write Table: "Output-oriented components of efficiency change in US agriculture (1960--2004) using the A-DEA index"
  ...
  # Write table  
  Tab_ADEA_TFPEDecomp_US_crs <- kable(data_ADEA_TPFEDecomp, booktabs = T, 
                                      digits = 2,
                                      row.names = FALSE,
                                      escape = FALSE,
                                      linesep = "",
                                      caption = "Output-oriented components of efficiency change in US agriculture (1960--2004) using the A-DEA index under CRS.",
                                      label = "Tab_ADEA_TFPEDecomp_US_crs",
                                      col.names = c('State', 
                                                    '1960', '2004', "$\\Delta$",
                                                    '1960', '2004', "$\\Delta$",
                                                    '1960', '2004', "$\\Delta$",
  ...
  # Print Latex file
  writeLines(Tab_ADEA_TFPEDecomp_US_crs, "Tables/Tab_ADEA_TFPEDecomp_US_crs.tex")
...
# --------------------------------------------- #
  ...
  ADEA1960 <- subset(ADEA.level_crs, year==1960)
  ADEA1970 <- subset(ADEA.level_crs, year==1970)
  ADEA1980 <- subset(ADEA.level_crs, year==1980)
  ADEA1990 <- subset(ADEA.level_crs, year==1990)
  ADEA2002 <- subset(ADEA.level_crs, year==2002)
  ...
  # Create table  
  Tab_ADEA_AvgRates_US_crs <- kable(data_table, booktabs = T, 
                                    digits = 2,
                                    row.names = FALSE,
                                    escape = FALSE,
                                    linesep = "",
                                    caption = "Average annual rates of growth (\\%) in TFP and efficiency in US agriculture (1960--2002) using the A-DEA index under CRS.",
                                    label = "Tab_ADEA_AvgRates_US_crs",
                                    col.names = c('State', 
                                                  'TFP', 'TFP*', 'OTE', 'OSME',
                                                  'TFP', 'TFP*', 'OTE', 'OSME',
                                                  'TFP', 'TFP*', 'OTE', 'OSME',
  ...
  # Print Latex file
  writeLines(Tab_ADEA_AvgRates_US_crs, "Tables/Tab_ADEA_AvgRates_US_crs.tex")
...
# ------------------------------------- #
  ...
  # Prepare the data
    ...
    for (i in idstates) {
      ADEA.cumulative_crs[[which(i==idstates)]] <- cbind(state = i, year=1960:2004,
    ...
    ADEA.US48_crs <- list()
    for (yr in 1960:2004) {
      ADEA.US48_crs[[which(yr == 1960:2004)]] <- c(year = yr, apply(ADEA.cumulative_crs[ADEA.cumulative_crs$year==yr,-c(1:2)],2,FUN = function(x) exp(mean(log(x)))))
    }
    ADEA.US48_crs <- as.data.frame(do.call(rbind, ADEA.US48_crs)) #turns the list into a data frame
    
    
    # For the efficiencies (last figure), we need arithmetic averages
    ADEA.US48_crs_eff <- ADEA.level_crs[,c("state", "year","TFPE", "OTE", "OME", "ROSE", "OSME")]
    ADEA.US48_crs_eff <- ADEA.US48_crs_eff %>% 
      group_by(year) %>% 
  ...
  tikz(file = "Figures/Fig_ADEA_US48_crs.tex", width = 7, height = 7)
