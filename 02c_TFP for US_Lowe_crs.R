...
# index under CRS using the state-level US data   #
#                                                 #
# ----------------------------------------------- #
...
Lowe.level_crs <- rbind(lowe_pacific, lowe_mountain, lowe_nplains, 
                        lowe_splains, lowe_cornbelt, lowe_southeast, 
                        lowe_northeast, lowe_lake, lowe_appalacian, 
                        lowe_delta)

# Order results by state
Lowe.level_crs <- Lowe.level_crs[order(Lowe.level_crs$state),]

# Save results for comparison in "03h_TFP for US_comparison"
save(Lowe.level_crs, file="R_output/Lowe.level_crs.Rda")

# Normalize with Base = AL 1960
Lowe.level_crs_norm <- Lowe.level_crs %>% 
  mutate_each(funs(./.[1]), setdiff(names(.), c("state","year")))


# ---------------------------------------------------------- #
#### Replicate figures from O'Donnell (2012) but with CRS ####
# ---------------------------------------------------------- #
  ...
  # -------------------------------------------------- #    
    ...
    tikz(file = "Figures/Fig_replication_crs.tex", width = 7, height = 7)
...
# ----------------------------------------------------------------- #
#### Replicate tables 2 and 3 from O'Donnell (2012) but with CRS ####
# ----------------------------------------------------------------- #  
  ...
  # Prepare data for the tables
    ...
    # PROFITABILITY 
      ...
      # PROF1960
      data_replication$PROF1960 <- Lowe.level_crs$PROF[Lowe.level_crs$year==1960]
      # PROF04
      data_replication$PROF2004 <- Lowe.level_crs$PROF[Lowe.level_crs$year==2004]
      # DPROF
    ...
    # TERMS OF TRADE 
      ...
      # TT1960
      data_replication$TT1960 <- Lowe.level_crs$TT[Lowe.level_crs$year==1960]
      # TT04
      data_replication$TT2004 <- Lowe.level_crs$TT[Lowe.level_crs$year==2004]
      # DTT
    ...
    # TOTAL FACTOR PRODUCTIVITY 
      ...
      # TFP1960
      data_replication$TFP1960 <- Lowe.level_crs$TFP[Lowe.level_crs$year==1960]
      # TFP04
      data_replication$TFP2004 <- Lowe.level_crs$TFP[Lowe.level_crs$year==2004]
      # DTFP
    ...
    # MAXIMM TOTAL FACTOR PRODUCTIVITY 
      ...
      # MP1960
      data_replication$MP1960 <- Lowe.level_crs$MP[Lowe.level_crs$year==1960]
      # MP04
      data_replication$MP2004 <- Lowe.level_crs$MP[Lowe.level_crs$year==2004]
      # DMP
    ...
    # TFP EFFICIENCY 
      ...
      # TFPE1960
      data_replication$TFPE1960 <- Lowe.level_crs$TFPE[Lowe.level_crs$year==1960]
      # TFPE04
      data_replication$TFPE2004 <- Lowe.level_crs$TFPE[Lowe.level_crs$year==2004]
      # DTFPE
    ...
    # OUTPUT TECHNICAL EFFICIENCY 
      ...
      # OTE1960
      data_replication$OTE1960 <- Lowe.level_crs$OTE[Lowe.level_crs$year==1960]
      # OTE04
      data_replication$OTE2004 <- Lowe.level_crs$OTE[Lowe.level_crs$year==2004]
      # DOTE
    ...
    # OUTPUT SCALE EFFICIENCY 
      ...
      # OSE1960
      data_replication$OSE1960 <- Lowe.level_crs$OSE[Lowe.level_crs$year==1960]
      # OSE04
      data_replication$OSE2004 <- Lowe.level_crs$OSE[Lowe.level_crs$year==2004]
      # DOSE
    ...
    # OUTPUT MIX EFFICIENCY 
      ...
      # OME1960
      data_replication$OME1960 <- Lowe.level_crs$OME[Lowe.level_crs$year==1960]
      # OME04
      data_replication$OME2004 <- Lowe.level_crs$OME[Lowe.level_crs$year==2004]
      # DOME
    ...
    # OUTPUT SCALE MIX EFFICIENCY 
      ...
      # OSME1960
      data_replication$OSME1960 <- Lowe.level_crs$OSME[Lowe.level_crs$year==1960]
      # OSME04
      data_replication$OSME2004 <- Lowe.level_crs$OSME[Lowe.level_crs$year==2004]
      # DOSME
    ...
    # Save US-average for comparison in 03h_TFP for US_comparison.R
      ...
      Summary_lowe_US_crs <- list(data_replication$DTFP[data_replication$state=="US48"],
                                  data_replication$DMP[data_replication$state=="US48"],
                                  data_replication$DTFPE[data_replication$state=="US48"])
      names(Summary_lowe_US_crs) <- c("TFP", "TC", "TFPE") 
      save(Summary_lowe_US_crs, file = "R_output/Summary_lowe_US_crs.Rda")
  ...
  # Write Table 2 from O'Donnell (2012): "Profitability, TFP, and efficiency change in US agriculture (1960-2004) using the Lowe index)
    ...
    # Write table  
    Tab_replication_ProfDecomp_crs <- kable(data_replication_ProfDecomp, booktabs = T, 
                digits = 2,
                row.names = FALSE,
                escape = FALSE,
                linesep = "",
                caption = "Profitability, TFP, and efficiency change in US agriculture (1960--2004) using the Lowe index under CRS.",
                label = "Tab_replication_ProfDecomp_crs",
                col.names = c('State', 
                              '1960', '2004', "$\\Delta$",
                              '1960', '2004', "$\\Delta$",
                              '1960', '2004', "$\\Delta$",
    ...
    # Print Latex file
    writeLines(Tab_replication_ProfDecomp_crs, "Tables/Tab_replication_ProfDecomp_crs.tex")
...
# Write Table 3 from O'Donnell (2012)
    ...
    # Create table  
    Tab_replication_TFPEDecomp_crs <- kable(data_replication_TPFEDecomp, booktabs = T, 
                       digits = 2,
                       row.names = FALSE,
                       escape = FALSE,
                       linesep = "",
                       caption = "Output-oriented components of efficiency change in US agriculture (1960--2004) using the Lowe index under CRS.",
                       label = "Tab_replication_TFPEDecomp_crs",
                       col.names = c('State', 
                                     '1960', '2004', "$\\Delta$",
                                     '1960', '2004', "$\\Delta$",
                                     '1960', '2004', "$\\Delta$",
    ...
    # Print Latex file
    writeLines(Tab_replication_TFPEDecomp_crs, "Tables/Tab_replication_TFPEDecomp_crs.tex")
...
# --------------------------------------------- #
  ...
  lowe1960 <- subset(Lowe.level_crs, year==1960)
  lowe1970 <- subset(Lowe.level_crs, year==1970)
  lowe1980 <- subset(Lowe.level_crs, year==1980)
  lowe1990 <- subset(Lowe.level_crs, year==1990)
  lowe2002 <- subset(Lowe.level_crs, year==2002)
...
# Write Table 4 from O'Donnell (2012): Average annual rates of growth in TFP and efficiency in US agriculture (1960-2002) using the Lowe index.
  ...
  # Create table  
  Tab_replication_AvgRates_crs <- kable(data_table, booktabs = T, 
                     digits = 2,
                     row.names = FALSE,
                     escape = FALSE,
                     linesep = "",
                     caption = "Average annual rates of growth (\\%) in TFP and efficiency in US agriculture (1960--2002) using the Lowe index under CRS.",
                     label = "Tab_replication_AvgRates_crs",
                     col.names = c('State', 
                                   'TFP', 'TFP*', 'OTE', 'OSME',
                                   'TFP', 'TFP*', 'OTE', 'OSME',
                                   'TFP', 'TFP*', 'OTE', 'OSME',
  ...
  # Print Latex file
  writeLines(Tab_replication_AvgRates_crs, "Tables/Tab_replication_AvgRates_crs.tex")
...
# ----------------------------------------- #
  ...
  # Prepare the data
    ...
    Lowe.cumulative_crs <- list()
    for (i in idstates) {
      Lowe.cumulative_crs[[which(i==idstates)]] <- cbind(state = i, year=1960:2004,
                                      sweep(Lowe.level_crs[Lowe.level_crs$state==Lowe.level_crs,-c(1:2)],MARGIN = 2,
                                            STATS = as.numeric(Lowe.level_crs[Lowe.level_crs$state==i,-c(1:2)][1,]),FUN = "/")) # for myself: 2 refers to column
    }
    Lowe.cumulative_crs <- do.call(rbind, Lowe.cumulative_crs) #turns the list into a data frame
    
    # Calculate US average using the geometric mean
    Lowe.US48_crs <- list()
    for (yr in 1960:2004) {
      Lowe.US48_crs[[which(yr == 1960:2004)]] <- c(year = yr, apply(Lowe.cumulative_crs[Lowe.cumulative_crs$year==yr,-c(1:2)],2,FUN = function(x) exp(mean(log(x)))))
    }
    Lowe.US48_crs <- as.data.frame(do.call(rbind, Lowe.US48_crs)) #turns the list into a data frame
  
  
    # For the efficiencies (last figure), we need arithmetic averages
    Lowe.US48_crs_eff <- Lowe.level_crs[,c("state", "year","TFPE", "OTE", "OME", "ROSE", "OSME")]
    Lowe.US48_crs_eff <- Lowe.US48_crs_eff %>% 
      group_by(year) %>% 
  ...
  #-------------------------------------------------------------#
    ...
    # -------------------------------------------------- #    
      ...
      # set filepath for latex output
      tikz(file = "Figures/Fig_replication_US48_crs.tex", width = 7, height = 7)
