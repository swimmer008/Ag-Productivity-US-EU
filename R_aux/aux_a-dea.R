# Data check for Färe-Primont index ----------

check.fareprim <- function(data, id.var, time.var, x.vars, y.vars,
  w.vars, p.vars) {
  ...
  if (is.null(w.vars) & is.null(p.vars)) {
    ...
    if (!(is.null(w.vars)) & !(is.null(p.vars))) {
      ...
      if (!(is.null(w.vars)) & is.null(p.vars)) {
        stop("Output prices p.vars must also be supplied",
          call. = FALSE)
      } else {
        stop("Input prices w.vars must also be supplied",
          call. = FALSE)
      }
    }
  }
...
# Färe-Primont first step (with technical change)
# ----------

fp.1 <- function(data, step1, year.vec, tech.reg, rts, orientation,
  parallel, PRICESO, PRICESI, mean.x, mean.y) {
      ...
      res2 <- foreach(dmu = 1:ncol(X1), .combine = rbind) %dopar%
        {
          ...
          if (length(step1) == 6) {
            ...
            res1 <- c(AO = AO, AI = AI, TFP = TFP,
            MP = MP, TFPE = TFPE, OTE = OTE, OSE = OSE,
            OME = OME, ROSE = ROSE, OSME = OSME,
            RME = RME, RE = RE, PRICEI = SHI, PRICEO = SHO)
          }
          } else {
          if (orientation == "in") {
            ...
            res1 <- c(REV = REV, COST = COST, PROF = PROF,
              ...
              ISE = ISE, IME = IME, RISE = RISE,
              ISME = ISME, RME = RME, CE = CE, PRICEI = SHI,
              PRICEO = SHO)
            } else {
            res1 <- c(AO = AO, AI = AI, TFP = TFP,
              MP = MP, TFPE = TFPE, ITE = ITE, ISE = ISE,
              IME = IME, RISE = RISE, ISME = ISME,
...
# Färe-Primont first step (without technical change)
# ----------

fp.2 <- function(data, step1, rts, orientation, PRICESO, PRICESI,
    ...
    if (orientation == "out") {
      ...
      if (length(step1) == 6) {
        ...
        res1 <- c(REV = REV, COST = COST, PROF = PROF,
          ...
          RE = RE, PRICEI = SHI, PRICEO = SHO)
      } else {
        res1 <- c(AO = AO, AI = AI, TFP = TFP, MP = MP,
          TFPE = TFPE, OTE = OTE, OSE = OSE, OME = OME,
      ...
      if (orientation == "in") {
        ...
        if (length(step1) == 6) {
          ...
          res1 <- c(AO = AO, AI = AI, TFP = TFP, MP = MP,
          TFPE = TFPE, ITE = ITE, ISE = ISE, IME = IME,
          RISE = RISE, ISME = ISME, RME = RME, CE = CE,
          PRICEI = SHI, PRICEO = SHO)
        }
      }
    }
...
# Färe-Primont first step (with window and technical
# change) ----------

fp.3 <- function(data, step1, year.vec, tech.reg, rts, orientation,
